...
- CLI framework: **commander**. Commands: `init`, `start`, `stop`, `record`, `test`, `report`.
...
5. **Resolver**: signal strategies (semantic via local-embedding cosine search, affordance, context, structure, index) + router — deterministic. Wire the embedding model + SQLite embedding cache. Golden-case tests port the tuned behavior documented in `DISCUSSION.md`.
6. **Grounding + Normalize + resolve + cache** with the `band ≥ medium` accept gate. *(ADR-002 #2–3)*
7. **Execution adapter**: cached-selector → resolver-heal → stale; runtime algorithmic heal loop. *(ADR-002 #4–5)*
